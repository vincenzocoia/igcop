...
$$\hat{C}_{IGL}(u, v; k) = u \Psi_{k}(u ^ {-1} \Psi_{k} ^ {\leftarrow}(v)).$$
...
$$\hat{C}_{\text{IG}}(u, v; k, \theta) = u H_{k}\left(H_{k} ^ {\leftarrow}(v; \theta); \theta u).$$
...
\frac{\Psi_k'\left(u^{-1} \Psi_k ^ {\leftarrow}(v)\right)}
     {\Psi_k'\left(\Psi_k^{\leftarrow}\left(v\right)\right)}$$
...
$$c_{\text{IGL}}(u, v; k) = 
-\frac{\Psi_k^{\leftarrow}(v) \Psi_k''\left(u^{-1} \Psi_k^{\leftarrow}(v)\right)}
