...
```{r}
u2 <- purrr::map(cpar, ~ {
    interp_kappa(interp_kappa_inv(u, eta = .x[1], k = .x[2]), eta = .x[1], k = .x[2])
})
tibble(cpar = cpar,
       u_original = uu,
       u_through_inverse = u2) %>%
...
```{r}
tau2 <- map(cpar, ~ pcondigcop(qcondigcop(tau, u, cpar = .x), u, cpar = .x))
tibble(cpar = cpar,
       tau_original = tautau,
       tau_through_the_inverse = tau2) %>%
    unnest_wider(cpar, names_sep = "_") %>%
    unnest(starts_with("tau_")) %>%
    mutate(diff = abs(tau_through_the_inverse - tau_original)) %>%
    DT::datatable()
test_that("Quantile function is inverse of pcond", {
    expect_equal(tau2, tautau)
})
```
...
digcop_numerical <- function(u, v, cpar, eps = 1.e-5) {
