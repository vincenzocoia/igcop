...
tibble(cpar = cpar,
    ...
    mutate(diff = u_original - u_through_inverse) %>% 
    DT::datatable()
...
```{r}
problem_cpar <- c(155.5, 4.1)
problem_u <- 0.9
f <- function(x) ((interp_kappa(exp(x) + 1, eta = problem_cpar[1], k = problem_cpar[2])) - 
                      (problem_u)) ^ 2
exp(nlm(f, -5)$estimate) + 1
interp_kappa(exp(nlm(f, -5)$estimate) + 1, problem_cpar[1], problem_cpar[2])
curve(f,-20, -4)

interp_kappa_inv_uniroot(u[[1]], eta = cpar[[2]][1], k = cpar[[2]][2])
f <- function(x) interp_kappa(x, eta = cpar[[2]][1], k = cpar[[2]][2])
dat <- tribble(
    ~iter, ~t,
1 , 5.862021 ,
2 , 3.761629 ,
...
ggplot(tibble(x = c(1, 10)), aes(x)) +
    stat_function(fun = f) +
    geom_hline(yintercept = u[1]) +
    geom_point(data = dat, mapping = aes(t, f, colour = iter)) +
...
```{r}
u2 <- purrr::map(cpar, ~ {
    interp_kappa(interp_kappa_inv_uniroot(u, eta = .x[1], k = .x[2]), eta = .x[1], k = .x[2])
})
tibble(cpar = cpar,
       u_original = uu,
       u_through_inverse = u2) %>%
    ...
    mutate(diff = u_original - u_through_inverse) %>% 
    DT::datatable()
test_that("Inverse of interpolating function with kappa works", {
    expect_equal(u2, uu)
})
```
...
### Computation of `pcondigcop`


Does this computation match an alternative, less simplified version of the calculation?
...
tibble(cpar = cpar,
    ...
    mutate(diff = pcondigcop_original - pcondigcop_alternative) %>% 
    DT::datatable()
...
Is it the inverse of the conditional cdf?
...
### Calculate qcondigcop looping across tau and u

```{r}
qcondigcop_looped <- function(tau, u, cpar) {
    qcond <- numeric()
    for (i in 1:length(u)) {
        qcond[i] <- qcondigcop(tau[i], u[i], cpar)
    }
    qcond
}
qcond1 <- purrr::map(cpar, ~ qcondigcop(tau, u, .x))
qcond2 <- purrr::map(cpar, ~ qcondigcop_looped(tau, u, .x))
tibble(qcond_vectorized = qcond1,
       qcond_looped = qcond2,
       cpar = cpar) %>%
    unnest(starts_with("qcond")) %>%
    unnest_wider(cpar, names_sep = "_") %>%
    DT::datatable()
test_that("Vectorization works for digcop", {
    expect_equal(qcond1, qcond2)
})
```
